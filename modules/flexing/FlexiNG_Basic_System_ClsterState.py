# -*- coding: utf-8 -*-
u"""
检查FlexiNG 各Cluster的状态
  - 检查指令:
      nginfo -s
    
    该指令列出当前各Cluster当前运行状态。
...
## Mandatory variables 
##--------------------------------------------
#module_id = 'NSTCN_2016071801'
module_id = 'FlexiNG_Basic_System_ClsterState'
tags      = ['china', 'flexing']
priority  = 'minor'
name      = u"FlexiNG Cluster状态检查"
desc      = __doc__
criteria  = u"""
（1）检查NG版本为 ['3.1','3.2','15','16'] 或者更高版本。
（2）检查各Cluster状态，并输出状态列表。
...
def printNgLockStatusList(ng_unit_list,status_str):
	UnitMsg = ''
	for unitNameid in range(0,len(ng_unit_list),3):
		UnitMsg += '\n-  '+ng_unit_list[unitNameid]+' , '+status_str
	return UnitMsg
...
#        vstatus= CheckStatus.VERSION_MATCHED
    ...
    ##网元版本符合条件
        ...
        #   
        _msg = ''
        ngUnitPatt = r'^\s*(/\w+\S+)\s+([N]?OK)\s*$'
        ngAppLockPatt = r'^\s*(/\w+\S+)\s+NOK\s*(LOCKED)\s*DISABLED\s*IDLE\s*(OK)\s*$'
        return_info_list = Find_NG_Patt_Return_Info_List(logfile,"nginfo -s",ngUnitPatt,2)
        #print 'return_info_list = ',return_info_list
        return_lock_info_list = Find_NG_Patt_Return_Info_List(logfile,"nginfo\s+-s",ngAppLockPatt,3)
        #print 'return_lock_info_list = ',return_lock_info_list
        ...
        if nokUnitMsg == '':
        	...
        	nokUnitMsg_1 = u"- 检查到NG部分Cluster状态异常"
        	errmsg = nokUnitMsg_1 + nokUnitMsg
        	status=CheckStatus.FAILED
        okUnitMsg_1 = u"- 检查到NG的版本为'%s',它属于%s或者更高版本" % (version_str,target_version)
        lockUnitMsg = printNgLockStatusList(return_lock_info_list,"LOCK")
        _msg = okUnitMsg_1 + okUnitMsg + lockUnitMsg
