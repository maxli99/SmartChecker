...
# @Last Modified time: 2016-03-11 16:35:11
...
check_commands = [
    ...
    ('ZDDE:IPDU,:"cat /opt/mme/conf/mmeGTPLBS-0x0968.ini",:;',"print the file content of LNX968NX.INI"),
]
...
def is_throttling_on(logs):
    ...
    if len(logs) == 0:
        __ERROR_MSG = u"日志文件中无法找到需要检测的信息，请确所认收集日志是是否执行“ZIBT:,,LNX968NX,INI,,,,A,;”。"
        raise()
    ...
    if status == CheckStatus.UNKNOWN:
        __ERROR_MSG = u"日志文件中无法找到需要检测的信息，请确认所收集的日志是否正确。"
        raise()
    return (status, info)
...
def run(logfile):
    ...
    try:
        logspt = LogSpliter()
        logspt.load(logfile)
        logs = logspt.get_log("DDE:IPDUcat /opt/mme/conf/mmeGTPLBS-0x0968.ini", fuzzy=True)
